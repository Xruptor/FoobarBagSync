...
local Search = FBS:NewModule("Search")
...
function Search:OnEnable()
	...
	SearchFrame:SetTitle("FoobarBagSync")
	...
	hooksecurefunc(warningframe, "Show" ,function()
		--always show the warning frame on the right of the FoobarBagSync window
	...
	warningframe:AddChild(ddlist)
	
	ddlist:SetWidth(300)
	
	--populate the dropdown
	hooksecurefunc(SearchFrame, "Show" ,function()
		local tmp = {}
		
		for realm, rd in pairs(FBS_DB) do
			for k, v in pairs(rd) do
				local key = k.."^"..realm
				tmp[key] = k.." - "..(FBS_REALMKEY[realm] or realm)
			end
		end
	...
	ddlist:SetCallback("OnValueChanged", function()

		local yName, yRealm  = strsplit("^", ddlist:GetValue())
		
		for realm, rd in pairs(FBS_DB) do
			if realm == yRealm then
				for k, v in pairs(rd) do
					if k == yName and v.guild and v.faction and v.realm then

						FBS.currentPlayer = k
						FBS.currentRealm = v.realm
						FBS.playerClass = v.class
						FBS.playerFaction = v.faction
						
						print("Player Changed: "..k.." - "..FBS_REALMKEY[realm])
						break
					end
				end
			end
...
function Search:DoSearch(searchStr)
	...
	if strlen(searchStr) > 0 then
								...
								for slotID, itemValue in pairs(bagInfo) do
									...
									if dblink then
										...
										if dName then
											...
											if playerSearch and string.sub(searchStr, 2) == q and string.sub(searchStr, 2) ~= "guild" and yName == FBS.currentPlayer and not tempList[dblink] then
						...
						if not previousGuilds[gName] and not previousGuildsXRList[trueRealmList] then
							...
							for q, r in pairs(xDBGuild[v.realm][guildN]) do
								...
								if dblink then
									local dName, dItemLink, dRarity, _, _, _, _, _, _, dTexture = GetItemInfo("item:"..dblink)
									if dName then
										if playerSearch and string.sub(searchStr, 2) == "guild" and xDBPlayer.guild and guildN == xDBPlayer.guild and not tempList[dblink] then
											table.insert(searchTable, { name=dName, link=dItemLink, rarity=dRarity, texture=dTexture } )
											tempList[dblink] = dName
											count = count + 1
										--we found a match
		...
		if countWarning > 0 then
			self.warninglabel:SetText("Missing Items: "..countWarning)
		end
